...
import viewcontroller.BoardRulesController;
import viewcontroller.BoardRulesView;
import viewcontroller.ChooseCardsController;
import viewcontroller.ChooseCardsView;
import viewcontroller.MainView;
import viewcontroller.MainViewController;
import viewcontroller.WizardObservable;
...
public class Program {
	...
	public static void main(String [] args){
		...
		Vector<WizardObservable> wizardControllers = new Vector<WizardObservable>();
		Vector<JPanel> wizardViews = new Vector<JPanel>();

		Vector<Card> allCards = Card.getAllCards();
		String [] cards = new String[ allCards.size() ];
		for( int index = 0; index < allCards.size(); index++ ) {
			cards[index] = allCards.elementAt(index).getName();
		}
		...
		ChooseCardsView chooseYourCardsView = new ChooseCardsView( "Select Your Cards" );
		wizardViews.addElement(chooseYourCardsView);
		ChooseCardsController chooseYourCardsController = new ChooseCardsController( cards, chooseYourCardsView, human );
		wizardControllers.addElement(chooseYourCardsController);
		
		ChooseCardsView chooseComputersCardsView = new ChooseCardsView( "Select The Computer's Cards" );
		wizardViews.addElement(chooseComputersCardsView);
		ChooseCardsController chooseComputersCardsController = new ChooseCardsController( cards, chooseComputersCardsView, computer );
		wizardControllers.addElement(chooseComputersCardsController);
		
		BoardRulesView boardRulesView = new BoardRulesView();
		wizardViews.addElement(boardRulesView);
		BoardRulesController boardRulesController = new BoardRulesController(boardRulesView, board, computersTurn);
		wizardControllers.addElement(boardRulesController);
		
		MainView mainView = new MainView( "FF8 Card Bot Helper" );
		MainViewController mainController = new MainViewController( mainView, wizardControllers,
				wizardViews, board, computersTurn );
